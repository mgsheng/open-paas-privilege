...
<html>
<head>
    <title>del-privilege-user-role(interface)</title>
...
<div class="panel panel-default">
    <div class="panel-body">
        <div ng-controller="AuthorizationCodeCtrl" class="col-md-10">
            ...
            <form action="delPrivilegeUserRole" method="post" class="form-horizontal">
                <input type="hidden" name="delPrivilegeUserUri" id="delPrivilegeUserUri" value="${delPrivilegeUserUri}"/>
                ...
                <div ng-show="visible">
                    ...
                    <div class="well well-sm">
                        ...
                        <div class="text-primary" id="delPrivilegeUser"></div> 
                    </div>
                </div>
                <br/>
                <br/>
                <button type="submit" class="btn btn-primary">调用用户角色删除接口</button>
...
<script type="text/javascript">
	function btnSubmit(){
	    ...
	    var delPrivilegeUserUri=$("#delPrivilegeUserUri").val();
		if(appId=='' || appUserId==''){
		    alert("请输入必传参数");
			return;
		}
		...
		$.post("${contextPath}/getSignature",
			...
			function(data){
				if(data.flag){
					...
					var uri=delPrivilegeUserUri+"?"+"appId="+appId+"&appUserId="+appUserId+"&createUser="+createUser+"&createUserId="+createUserId
							+"&appKey="+appKey+"&signature="+signature+"&timestamp="+timestamp+"&signatureNonce="+signatureNonce;
					$("#delPrivilegeUser").html(uri);
				}else{
				    jQuery("#delPrivilegeUser").html('无效数据，请重新申请');
				}
			}
 		); 
	}
