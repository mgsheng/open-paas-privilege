...
<html>
<head>
    <title>get-privilege-user-role(interface)</title>
...
<div class="panel panel-default">
    <div class="panel-body">
        <div ng-controller="AuthorizationCodeCtrl" class="col-md-10">
            ...
            <form action="getPrivilegeUserRole" method="post" class="form-horizontal">
                <input type="hidden" name="getPrivilegeUserUri" id="getPrivilegeUserUri" value="${getPrivilegeUserUri}"/>
                ...
                <div ng-show="visible">
                    ...
                    <div class="well well-sm">
                        ...
                        <div class="text-primary" id="getPrivilegeUser"></div> 
                    </div>
                </div>
                <br/>
                <br/>
                <button type="submit" class="btn btn-primary">调用用户权限查询接口</button>
...
<script type="text/javascript">
	function btnSubmit(){
		...
		var appId=$("#appId").val();
	    var getPrivilegeUserUri=$("#getPrivilegeUserUri").val();
		if(appId=='' || appUserId==''){
		    alert("请输入必传参数");
			return;
		}
		...
		$.post("${contextPath}/getSignature",
			...
			function(data){
				if(data.flag){
					...
					var uri=getPrivilegeUserUri+"?"+"appUserId="+appUserId+"&appId="+appId
							+"&appKey="+appKey+"&signature="+signature+"&timestamp="+timestamp+"&signatureNonce="+signatureNonce;
					$("#getPrivilegeUser").html(uri);
				}else{
				    jQuery("#getPrivilegeUser").html('无效数据，请重新申请');
				}
			}
 		); 
	}
