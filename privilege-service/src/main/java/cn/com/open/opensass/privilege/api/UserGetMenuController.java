...
public class UserGetMenuController extends BaseControllerUtil {
	public static final String SIGN = RedisConstant.SIGN;

	private static final Logger log = LoggerFactory.getLogger(UserGetMenuController.class);
	@Autowired
	...
	public void getPrivilege(HttpServletRequest request, HttpServletResponse response,
		...
		//用户资源缓存
		PrivilegeAjaxMessage roleMessage = privilegeUserRedisService.getRedisUsersRole(privilegeUserVo.getAppId(),
				privilegeUserVo.getAppUserId());
		...
		{
			menuMessage=privilegeMenuService.getMenusRedis(privilegeUserVo.getAppId(),
					privilegeUserVo.getAppUserId());
		}
		//用户菜单缓存
		...
		if (roleMessage.getCode().equals("0")) {// code为0该用户不存在
			...
			// 去重处理
			Set<PrivilegeResourcesVo> privilegeResourceVos = new HashSet<PrivilegeResourcesVo>();
			// 如果为管理员 返回应用所有资源
			if (Type == 2) {
				...
				List<PrivilegeResourcesVo> resources = JSONArray.toList(objArray, PrivilegeResourcesVo.class);
				privilegeResourceVos.addAll(resources);
			} else {
				objArray = (JSONArray) obj1.get("resourceList");
				List<PrivilegeResourcesVo> userResourceVos = JSONArray.toList(objArray, PrivilegeResourcesVo.class);
		...
		if (menuMessage.getCode().equals("0")) {
			...
			if (boo) {// 有管理员角色获取所有应用下菜单
				...
				Set<PrivilegeMenusVo> menuSet = new HashSet<PrivilegeMenusVo>();
				if (Type == 2) {
					// 如果为管理员 返回应用所有菜单
					...
					List<PrivilegeMenusVo> menuList = JSONArray.toList(menuArray, PrivilegeMenusVo.class);
					Set<PrivilegeMenusVo> set = new HashSet<PrivilegeMenusVo>();
				...
				Set<PrivilegeMenusVo> privilegeMenuListData = privilegeMenuService.getAllMenuByUserIds(privilegeMenuList,
			...
			if (Type == 2) {
				...
				List<PrivilegeMenusVo> userMenuList=JSONArray.toList(objArray, PrivilegeMenusVo.class);
				menuSet.addAll(userMenuList);
			}
			menuMap.put("menuList", menuSet);
			map.putAll(menuMap);
		}
		  Set<PrivilegeMenusVo> menuSet=(Set<PrivilegeMenusVo>) map.get("menuList");
		    menuSet=MenuProcessUtil.processMenuCodes(menuSet, privilegeUserVo.getMenuCode());
			menuMap.put("menuList", menuSet);
			map.putAll(menuMap);
