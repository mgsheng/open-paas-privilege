...
//			menuJedis = redisDao.getUrlRedis(prefix, appId, appUserId);
	...
	@Override
	public PrivilegeAjaxMessage getManagerMenuRedis(Map<String, String> appId) {
		PrivilegeAjaxMessage ajaxMessage = new PrivilegeAjaxMessage();
		Map<String, Object> MenuMap = new HashMap<String, Object>();
	...
	@Override
	public List<PrivilegeMenuVo> getMenuVoListByAppId(Map<String, String> appId) {
		List<PrivilegeMenu> privilegeMenus=privilegeMenuRepository.getMenusByAppId(appId);
		List<PrivilegeMenuVo> privilegeMenuVos=new ArrayList<PrivilegeMenuVo>();
		for(PrivilegeMenu privilegeMenu:privilegeMenus){
			if (privilegeMenu!=null) {
				PrivilegeMenuVo privilegeMenuVo=new PrivilegeMenuVo();
	...
	@Override
	public PrivilegeAjaxMessage getMenuRedisByMap(Map<String, String> map) {

		PrivilegeAjaxMessage ajaxMessage = new PrivilegeAjaxMessage();
		/* 验证用户是否存在 */
		PrivilegeUser user = privilegeUserService.findByAppIdAndUserId(map.get("appId"),map.get("appUserId") );
		if (null == user) {
			ajaxMessage.setCode("0");
			ajaxMessage.setMessage("User Is Null");
			return ajaxMessage;
		}
		Integer version = (Integer) redisClientTemplate.getObject(appMenuVersionCache+map.get("appId"));
		int Type=1; // 角色类型，1-普通用户，2-系统管理员，3-组织机构管理员
		//判断用户角色是否是系统管理员  
		List<PrivilegeRole> roles = privilegeRoleService.getRoleListByUserIdAndAppId(map.get("appUserId"), map.get("appId"));
		for (PrivilegeRole role : roles) {
			if(role.getRoleType()!=null){
				if (role.getRoleType() == 2) {//若角色为系统管理员  
					if (role.getGroupId()!=null&&!role.getGroupId().isEmpty()) {//若该管理员为组织机构管理员
						...
						if ("1".equals(message.getCode())) {
							...
							JSONObject json=	JSONObject.fromObject(redisJson);
							json.put("version", version);
							redisClientTemplate.setString(prefix + map.get("appId") + SIGN + map.get("appUserId"), json.toString());
						}
						return message;
					}
				}
...
//		String menuJedis = redisDao.getUrlRedis(prefix, appId, appUserId);
		String menuJedis = redisClientTemplate.getString(prefix+map.get("appId")+  SIGN+map.get("appUserId"));
		if (null == menuJedis || menuJedis.length() <= 0)

		{	
			List<PrivilegeMenu> privilegeMenuList = getMenuListByUserId(map.get("appUserId"), map.get("appId"));
			//通过查找RoleResource  查找相应的菜单
			List<String> FunIds=new ArrayList<String>();
			List<PrivilegeRoleResource> rivilegeRoleResources=privilegeRoleResourceService.findUserRoleResources(map.get("appId"), map.get("appUserId"));
			for(PrivilegeRoleResource roleResource:rivilegeRoleResources){
				if (roleResource.getPrivilegeFunId()==null||("").equals(roleResource.getPrivilegeFunId())) {
					List<PrivilegeMenu> menus=getMenuListByResourceId(roleResource.getResourceId(),map.get("appId"));
					privilegeMenuList.addAll(menus);
				}else{
					//RoleResource 中functionId
					FunIds.add(roleResource.getPrivilegeFunId());
			...
			if (FunIds != null&&FunIds.size()>0) {
				for (String funIds : FunIds) {
					String[] functIds = funIds.split(",");
					List<PrivilegeMenu> menus = getMenuListByFunctionId(functIds,map.get("appId"));
					privilegeMenuList.addAll(menus);
				}
			}
...
//					System.out.println(menus.size());
//				}
				List<PrivilegeMenu> menus = getMenuListByResourceId2(resIds,map.get("appId"));
				privilegeMenuList.addAll(menus);
				//System.out.println(menus.size());
			}
			if (privilegeMenuList.size() <= 0) {
			...
			if (Type == 3) {//如果用户角色为组织机构管理员
				//把该组织机构拥有的菜单放入缓存
				PrivilegeAjaxMessage message = privilegeGroupService.findGroupPrivilege(user.getGroupId(), map.get("appId"));
				if (message.getCode().equals("1")) {
					JSONObject object=JSONObject.fromObject(message.getMessage());
					JSONArray array=object.getJSONArray("menuList");
					List<PrivilegeMenuVo> groupMenuList=JSONArray.toList(array, PrivilegeMenuVo.class);
...
//			PrivilegeUrl privilegeUrl = new PrivilegeUrl();
			Map<Object, Object> mapMenu = new HashMap<Object, Object>();
			mapMenu.put("menuList", privilegeMenuListData);
			if (version != null) {
				mapMenu.put("version", version);
			}
			menuJedis = new JSONObject().fromObject(mapMenu).toString();
...
//			menuJedis = redisDao.getUrlRedis(prefix, appId, appUserId);
		...
		if (null != menuJedis && menuJedis.length() > 0) {
			...
			if (version != null) {
				...
				Integer userMenuCacheVersions = (Integer) object.get("version");
				if (userMenuCacheVersions == null) {
					 ajaxMessage = updateMenuRedis(map.get("appId"), map.get("appUserId") );
				} else {
					if (version.equals(userMenuCacheVersions)) {
						ajaxMessage.setCode("1");
						ajaxMessage.setMessage(menuJedis);
