	...
	<script>
		...
		function findUsers(){
			...
			$('#dg').datagrid({
		        ...
		        columns:[[
							...
							{ field: 'DCENTERCODE', title: '大区ID',hidden:true,align:'center',sortable:true,width:'10%'}
							]],
		...
		function check(){
			...
			if (IDCARD == "" || IDCARD == null || regex_IdCard.test(IDCARD) != true) {
				$.messager.alert("系统提示","身份整号码不能为空或格式不正确，请重新填写！","error");	
				return false;
			}
			...
			$.post(url,
					{
						...
						"Name":Name,"SEX":SEX,"PHONENO":PHONENO,"type":'5',"IDCARD":IDCARD,"POSTCODE":POSTCODE,
						"MOBILEPHONE":MOBILEPHONE,"EMAIL":EMAIL,"HRLOGINNAME":HRLOGINNAME,"ADDRESS":ADDRESS
					},
                 	function(data){
                     	if (data.status == '1') {
